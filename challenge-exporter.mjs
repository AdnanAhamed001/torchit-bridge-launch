...
const generateTemplate = () => {
  const row1Cards = cardsData.slice(0, 3).map(generateCardColumn);
  const row2Cards = cardsData.slice(3, 6).map(generateCardColumn);

  const template = {
    "version": "0.4",
    "title": "Torchit Challenge Section",
    "type": "section",
    "content": [
      {
        "id": generateRandomId(),
  ...
  fs.writeFileSync('challenge-elementor.json', JSON.stringify(template, null, 2));
  console.log('Successfully generated challenge-elementor.json!');
};
